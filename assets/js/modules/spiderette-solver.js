...
// -----------------------------------------------------------------
// WHY SHUFFLE-THEN-VERIFY
// Every legal move in this variant places a card on a destination
// that is EXACTLY one rank higher (see canDrop()'s rule in
// spiderette.js — any suit, just rank+1). That means generating a
// deal by starting from 4 complete King-to-Ace runs and undoing
// random legal moves would always produce a tableau that's
// rank-sorted, card by card, top to bottom, in every column — flip a
// card and the one underneath is (almost always) exactly one rank
// higher, which reads as trivially solved rather than a real puzzle.
//
// Instead, deals are generated by shuffling for real, then verifying
// solvability with an actual solver, keeping the deal only if the
// solver proves it clearable — the standard approach real solitaire
// generators use (e.g. how Microsoft's Spider/FreeCell verify their
// daily deals):
//
//   repeat
//     seed = random()
...
// few implementation choices that matter a lot for how often it
...
//     generated (not 3 structurally-identical branches) — this keeps
//     the effective branching factor low.
...
// Roughly 45-50% of random seeds solve within a 150,000-node /
// ~1 second budget — good enough to solve live in a browser (a
// handful of attempts, well under a second on average, see
...
// Builds a full deck and Fisher-Yates shuffles it using the seeded PRNG
...
// -----------------------------------------------------------------
// Every legal move from the current tableau, with identical
// empty-column destinations collapsed into one
...
// True if a 13-card pile is a complete same-colour King-to-Ace run
...
// Deals the next wave of stock cards, one per column, left to right
...
// Fraction of vertically-adjacent card pairs that share a suit, across all columns
...
// Compact string key for a tableau state, used to dedupe visited nodes in the solver
...
export function solve(initialCols, initialStock, opts = {}) {
  ...
  // Recursive best-first search: tries the top-scoring candidate
  // moves/deals at each depth, backtracking if none lead to a solution.
