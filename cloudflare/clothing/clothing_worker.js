...
//   PUT  /clothing  { items } -> { items, updatedAt }  (overwrites)
// =================================================================
...
const KV_KEY = 'clothing';
const MAX_ITEMS = 300;
const MAX_TEXT_LENGTH = 200;       // name
const MAX_SIZE_LENGTH = 50;        // size (free text, e.g. "M", "42", "32/34")
...
const RATE_LIMIT_PREFIX = 'clothing';
...
/** Rejects anything that isn't a well-formed { items: [{id, person, name, url, size, description, rating, photo, addedAt}] } payload. */
function validateItems(body) {
  ...
  for (const raw of body.items) {
    ...
    const size = typeof raw.size === 'string' ? raw.size.trim().slice(0, MAX_SIZE_LENGTH) : '';
...
export default {
  async fetch(request, env) {
    ...
    if (!env.CLOTHING_KV) {
      return jsonResponse({ error: 'Server misconfigured: CLOTHING_KV binding ontbreekt' }, 500, headers);
    }
    ...
    if (url.pathname !== '/clothing') {
      return jsonResponse({ error: 'Not found' }, 404, headers);
    }

    // ---- GET /clothing : current state ----
    if (request.method === 'GET') {
      const stored = (await env.CLOTHING_KV.get(KV_KEY, 'json')) || { items: [], updatedAt: Date.now() };
      return jsonResponse(stored, 200, headers);
    }

    // ---- PUT /clothing : overwrite with the browser's current state ----
    if (request.method === 'PUT') {
      ...
      await env.CLOTHING_KV.put(KV_KEY, JSON.stringify(stored));
      return jsonResponse(stored, 200, headers);
    }
